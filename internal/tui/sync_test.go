...
func (f *fakeSyncer) PushRemote() error         { return f.pushRemoteErr }
func (f *fakeSyncer) Snapshot(msg string) error { return f.snapshotErr }
...
func TestAsyncPush_AfterMutation(t *testing.T) {
	syncer := &fakeSyncer{hasRemote: true}
	m := testDayModelWithSyncer(t, syncer)

	// Add a task so there's something to complete
	now := time.Now()
	due := now.Add(5 * time.Minute)
	m.svc.Add("test task", &due)
	m.refreshData()

	// Complete the task via space key
	got, cmd := m.Update(teaKey(" "))
	gm := got.(model)
	if !gm.pushBusy {
		t.Error("expected pushBusy=true after mutation with remote")
	}
	if cmd == nil {
		t.Error("expected non-nil cmd for async push")
	}
}

func TestAsyncPush_ResultClearsBusy(t *testing.T) {
	syncer := &fakeSyncer{hasRemote: true}
	m := testDayModelWithSyncer(t, syncer)
	m.pushBusy = true

	got, _ := m.Update(asyncPushResultMsg{})
	gm := got.(model)
	if gm.pushBusy {
		t.Error("expected pushBusy=false after asyncPushResultMsg")
	}
}

func TestAsyncPush_ErrorShowsStatus(t *testing.T) {
	syncer := &fakeSyncer{hasRemote: true}
	m := testDayModelWithSyncer(t, syncer)
	m.pushBusy = true

	got, _ := m.Update(asyncPushResultMsg{err: errors.New("fail")})
	gm := got.(model)
	if gm.syncStatus != "Push failed" {
		t.Errorf("expected syncStatus 'Push failed', got %q", gm.syncStatus)
	}
}

func TestAsyncPush_QuitWaitsForPush(t *testing.T) {
	syncer := &fakeSyncer{hasRemote: true}
	m := testDayModelWithSyncer(t, syncer)
	m.pushBusy = true

	got, _ := m.Update(teaKey("q"))
	gm := got.(model)
	if !gm.quitting {
		t.Error("expected quitting=true when pushBusy")
	}
}

func TestAsyncPush_QuitQuitsWhenDone(t *testing.T) {
	syncer := &fakeSyncer{hasRemote: true}
	m := testDayModelWithSyncer(t, syncer)
	m.pushBusy = true
	m.quitting = true

	_, cmd := m.Update(asyncPushResultMsg{})
	if cmd == nil {
		t.Error("expected tea.Quit cmd when quitting and push completes")
	}
}

func TestAsyncPush_NoRemoteNoCmd(t *testing.T) {
	syncer := &fakeSyncer{hasRemote: false}
	m := testDayModelWithSyncer(t, syncer)
